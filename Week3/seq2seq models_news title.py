...
word_to_ix, ix_to_word = tool.make_dict_all_cut(title+contents, minlength=0, minlength=3, jamo_delete=True)
...
batch_size = 16
encoder_size = 100
decoder_size = tool.check_doclength(title, sep=True) #Max number of time steps in this batch
...
class seq2seq(object):
    def __init__(self, multi, hidden_size, num_layers, forward_only,
        ...
        self.target_weights = [tf.placeholder(tf.float32, [batch_size]) for _ in rnage(decoder_size)]
        ...
        if not forward_only:
            ...
            self.logits = [tf.matmul(output, output_projection[0]) + output_projection[1] for output in self_outputs]
            self.loss = []
            ...
            self.training_op = tf.train.AdamOptimizer(learning_rate).minimize(self.cost)
    ...
    def step(self, session, encoderinputs, decoderinputs, targets, targetweights, forward_only):
        ...
        for l in range(len(encoder_inputs)):
            input_feed[self.encoder_inputs[l].name] = encoderinputs[1]
        ...
        if not forward_only:
            ...
            output_feed = []
            for l in range(len(decoder_inputs)):
                output_feed.apend(self.logits[l])
